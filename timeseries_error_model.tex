...
In practice, these measurement have systematic errors
caused by fouling and calibration drift,
...
The noise is modeled with a Gaussian distribution,
%
\begin{equation}
...
For the fouling check, the measuring $m$ and reference $r$
...
\end{equation}
%
Let $Y$ and $Z$ be the principle and reference measurements,
...
\end{equation}
%
The superscripts are necessary because the value of $X$
will depend on the calibration of either device.
...
\begin{align}
    E_f &= (Y_b - Y_a) - (Z_b - Z_a) \notag \\
        ...
        &= (E^m_{b} - E^m_{a}) - (E^r_{b} - E^r_{a})xt{}
    \text{.}
\end{align}
%
...
(e.g., time averaging),
such that its error is perfectly correlated during the check,
