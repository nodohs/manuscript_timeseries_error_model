...
\section{Derivation of the Shrinkage Weight for Diffusion Rate}

The shrinkage weight $\hat{w}_j$ in the empirical-Bayes estimator for the diffusion rate
follows from the conjugate inverse-gamma prior structure.
We assume an inverse-gamma prior on the diffusion variance,
%
\begin{equation}
    \sigma_j^2 \sim \operatorname{IG}(\kappa, \theta)
    \text{,}
\end{equation}
%
where $\kappa > 0$ is the shape parameter and $\theta > 0$ is the scale parameter.
The sample variance $s_j^2$ has a scaled chi-squared distribution,
%
\begin{equation}
    \frac{K_j s_j^2}{\sigma_j^2} \sim \chi^2_{K_j}
    \text{,}
\end{equation}
%
where $K_j$ is the number of checks for device $j$.

The inverse-gamma prior has density
%
\begin{equation}
    p(\sigma_j^2) \propto (\sigma_j^2)^{-\kappa-1} \exp\left(-\frac{\theta}{\sigma_j^2}\right)
    \text{,}
\end{equation}
%
...
%
\begin{equation}
    p(s_j^2 \mid \sigma_j^2) \propto (\sigma_j^2)^{-K_j/2} \exp\left(-\frac{K_j s_j^2}{2\sigma_j^2}\right)
    \text{.}
\end{equation}
%
...
%
\begin{equation}
    p(\sigma_j^2 \mid s_j^2) \propto p(s_j^2 \mid \sigma_j^2) \, p(\sigma_j^2)
    \text{.}
\end{equation}
%
Substituting the densities yields
%
\begin{align}
    p(\sigma_j^2 \mid s_j^2)
    &\propto (\sigma_j^2)^{-K_j/2} \exp\left(-\frac{K_j s_j^2}{2\sigma_j^2}\right)
             (\sigma_j^2)^{-\kappa-1} \exp\left(-\frac{\theta}{\sigma_j^2}\right)
    \notag \\
    &= (\sigma_j^2)^{-(\kappa + K_j/2)-1} \exp\left(-\frac{\theta + K_j s_j^2/2}{\sigma_j^2}\right)
    \text{,}
\end{align}
%
which is another inverse-gamma distribution,
%
\begin{equation}
    \sigma_j^2 \mid s_j^2 \sim \operatorname{IG}\left(\kappa + \frac{K_j}{2},\, \theta + \frac{K_j s_j^2}{2}\right)
    \text{.}
\end{equation}
%
...
%
\begin{align}
    \mathbb{E}[\sigma_j^2 \mid s_j^2]
    &= \frac{\theta + K_j s_j^2/2}{\kappa + K_j/2 - 1}
    \notag \\
    &= \frac{\theta}{\kappa + K_j/2 - 1} + \frac{K_j s_j^2/2}{\kappa + K_j/2 - 1}
    \notag \\
    &= \frac{\theta}{\kappa - 1} \cdot \frac{\kappa - 1}{\kappa + K_j/2 - 1}
       + s_j^2 \cdot \frac{K_j/2}{\kappa + K_j/2 - 1}
    \text{.}
\end{align}
%
Recognizing that $\bar{\sigma}^2 := \theta/(\kappa-1)$ is the prior mean
and rearranging terms gives
%
\begin{equation}
    \mathbb{E}[\sigma_j^2 \mid s_j^2]
    = \frac{K_j/2}{\kappa + K_j/2 - 1} s_j^2
      + \frac{\kappa - 1}{\kappa + K_j/2 - 1} \bar{\sigma}^2
    \text{.}
\end{equation}
%
Multiplying numerator and denominator by 2 yields
%
\begin{equation}
    \mathbb{E}[\sigma_j^2 \mid s_j^2]
    = \frac{K_j}{K_j + 2(\kappa - 1)} s_j^2
      + \frac{2(\kappa - 1)}{K_j + 2(\kappa - 1)} \bar{\sigma}^2
    \text{.}
\end{equation}
%
In practice, we substitute the empirical-Bayes estimates
$\hat{\kappa}$ and $\hat{\theta}$ obtained by maximizing Equation~\ref{eq:hierarchical_likelihood}
in place of the population parameters $\kappa$ and $\theta$.
Defining the shrinkage weight
%
\begin{equation}
...
\end{equation}
%
and the empirical-Bayes estimate of the population mean
%
\begin{equation}
    \bar{\hat{\sigma}}^2 := \hat{\theta}/(\hat{\kappa}-1)
    \text{,}
...
%
\begin{equation}
    \hat{\sigma}_j^2 = \hat{w}_j s_j^2 + (1 - \hat{w}_j) \bar{\hat{\sigma}}^2
    \text{,}
\end{equation}
%
which shrinks the sample variance $s_j^2$ toward the population mean $\bar{\hat{\sigma}}^2$.
The weight $\hat{w}_j$ increases with $K_j$ (more checks provide stronger evidence)
and decreases as $\hat{\kappa}$ increases (stronger prior belief in population homogeneity).


\section{Derivation of the Shrinkage Weight for Drift Rate}

The shrinkage weight $\hat{u}_j$ in the empirical-Bayes estimator for the drift rate
follows from the conjugate Gaussian prior structure.
We assume a Gaussian prior on the drift rate,
%
\begin{equation}
    \mu_j \sim \mathcal{N}(\nu, \tau^2)
    \text{,}
\end{equation}
%
and the maximum-likelihood estimate $\hat{\mu}_j$ has sampling distribution
%
\begin{equation}
    \hat{\mu}_j \mid \mu_j \sim \mathcal{N}\left(\mu_j, \frac{\hat{\sigma}_j^2}{K_j}\right)
    \text{,}
\end{equation}
%
where $K_j$ is the number of checks and $\hat{\sigma}_j^2$ is the estimated diffusion variance.

By Bayes' theorem, the posterior distribution is
%
\begin{equation}
    p(\mu_j \mid \hat{\mu}_j) \propto p(\hat{\mu}_j \mid \mu_j) \, p(\mu_j)
    \text{.}
\end{equation}
%
Substituting the Gaussian densities and completing the square yields another Gaussian,
%
\begin{equation}
    \mu_j \mid \hat{\mu}_j \sim \mathcal{N}(\tilde{\mu}_j, \tilde{\sigma}_j^2)
    \text{,}
\end{equation}
%
...
\begin{align}
    \tilde{\mu}_j
    &= \frac{\tau^{-2} \nu + (K_j/\hat{\sigma}_j^2) \hat{\mu}_j}{\tau^{-2} + K_j/\hat{\sigma}_j^2}
    \notag \\
    &= \frac{K_j\hat{\sigma}_j^{-2}}{K_j\hat{\sigma}_j^{-2} + \tau^{-2}} \hat{\mu}_j
       + \frac{\tau^{-2}}{K_j\hat{\sigma}_j^{-2} + \tau^{-2}} \nu
    \notag \\
    &= \frac{K_j\tau^2}{K_j\tau^2 + \hat{\sigma}_j^2} \hat{\mu}_j
       + \frac{\hat{\sigma}_j^2}{K_j\tau^2 + \hat{\sigma}_j^2} \nu
    \text{.}
\end{align}
%
In practice, we substitute the empirical-Bayes estimates
$\hat{\nu}$ and $\hat{\tau}^2$ from the main text
in place of the population parameters $\nu$ and $\tau^2$.
Defining the shrinkage weight
%
\begin{equation}
...
%
\begin{equation}
    \tilde{\mu}_j = \hat{u}_j \hat{\mu}_j + (1 - \hat{u}_j) \hat{\nu}
    \text{,}
\end{equation}
%
which shrinks the individual estimate $\hat{\mu}_j$ toward the population mean $\hat{\nu}$.
The weight $\hat{u}_j$ increases with $K_j$ (more checks provide stronger evidence)
and decreases with $\hat{\sigma}_j^2$ (higher diffusion variance implies less precise estimates).
