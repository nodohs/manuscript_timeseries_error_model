...
\begin{abstract}
    Environmental time-series measurements are subject to systematic errors
    caused by fouling and calibration drift.
    Consequently, operators must periodically check, clean, and recalibrate their measurement devices.
    By modeling error growth between these checks as a stochastic process,
    we derive bias corrections and confidence intervals for time-series measurements.
\end{abstract}
...
%
Environmental monitoring often involves collecting in situ measurements 
over extended deployment periods.
In practice, these measurements are affected by systematic errors
caused by fouling and calibration drift of the measurement device,
as well as irreducible noise.
To quantify that error, the device is periodically checked against known references.
During each check, the device may also be cleaned or recalibrated,
resetting the corresponding error component to zero.

Later on, this information is used to correct the raw measurements,
usually by linearly interpolating the errors between successive checks \citep{Wagner_2006}.
Mathematically, this procedure is equivalent to treating the error as
as a Wiener process and conditioning on the check measurements.
As the checks are also imprecise, we treat them as noisy observations of the error process,
and propagate their uncertainty through timeseries.
...
We also show how to extend this framework to multi-point calibrations and 
monitoring networks consisting of similar devices.
The resulting method provides transparent, analytically tractable estimates of
bias corrections and confidence intervals,
while remaining consistent with operational procedures used in environmental monitoring.
...
\end{equation}
%
where $\%E$ expresses the error as a percentage of the measured value.
However, it is simpler to work in terms of the fractional error,
%
\begin{equation}
...
before and after cleaning the device, $Y_b$ and $Y_a$ repsectively.
Under steady conditions,
the fouling error is given by the difference between the check measurements,
%
\begin{equation}
    E_f = Y_b - Y_a
    \qquad \text{or} \qquad
...
\end{equation}
%
Otherwise, the fouling check must be adjusted 
using a reference device to measure the environment
before and after cleaning, $Z_b$ and $Z_a$,
%
\begin{equation}
    E_f = (Y_b - Y_a) - (Z_b - Z_a)
    \qquad \text{or} \qquad
...
Recall that the checks measurements have their own uncertainty,
which must be propagated through the error model.
We will review the general approach,
before applying it to fouling and calibration-drift.

Formally, we define a Brownian bridge with uncertain endpoints,
...
\subsection{Multipoint Calibration}

% cite Guidelines for data acquisition and data quality evaluation in environmental chemistry
