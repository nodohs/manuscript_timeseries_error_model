...
within the precision of the device.
In standard practice, information from these checks is later used to correct
the raw measurements by linearly interpolating estimated errors
between check events \citep{Wagner_2006}.
...
Under this assumption, we derive closed-form expressions for the conditional mean
and variance of the error,
which correspond to a special case of time-continuous Kalman filtering and smoothing.
We then extend the model to multipoint calibrations
and monitoring networks of similar devices.
The result is a transparent and tractable framework for estimating
fouling and drift corrections with associated confidence intervals
...
We assume the fouling or calibration-drift error evolves as a continuous-time
stochastic process, defined by
%
\begin{equation}
    E_t = \delta_{k-1} E_{k-1} + \mu (t - t_{k-1}) + \sigma \left(W_t - W_{t_{k-1}}\right)
    \text{,}
\end{equation}
%
...
and $W_t$ is a standard Wiener process, so its increment satisfies
%
\begin{equation}
    W_t - W_{t_{k-1}} \sim \mathcal{N}(0,\, t - t_{k-1})
...
Also note that the reset is ``soft'' in the sense that
the error is not known exactly:
when $\delta_{k-1}=0$ we set $\mathbb{E}[E_{k-1}]=0$ but retain the check variance
$\operatorname{Var}[E_{k-1}]=v_{k-1}$.

The time series $E_t$ is subdivided into intervals that are bracketed by check measurements.
...
%
The conditional mean and variance can be written without superscripts
($m^o$, $m^c$, etc.) as
%
\begin{align}
...
%
\begin{equation}
    m_{k-1} := \mathbb{E}\left[ E_{k-1}\right] = e_{k-1}
    \text{,} \qquad
    m_{k} := \mathbb{E}\left[ E_{k}\right] = e_{k}
    \text{,} \qquad
...
\caption{
    Error variance during open and closed intervals
    with diffusion rate $\sigma^2 = 0.02$
    and check variances $v(t=0) = 0.04$ and $v(t=10) = 0.09$.
    The two variance components represent uncertainty from the check measurements
    and from evolution of the Wiener-process error.
}
...
The next sections derive variance formulas for the
...
\end{equation}
%
Similarly for the reference device if it also uses filtering; otherwise,
we assume its errors are independent,
...
the standard and measurement device have known precisions $\sigma_s$ and $\sigma_m$
...
We assume the error diffusion in time is independent of the diffusion across the $y$ dimension,
so the two bridges evolve independently.
...
the conditional mean of the $y$-bridge at time $t_k$ is
a linear interpolation between the standards,
%
\begin{align}
...
\end{align}
%
where $\lambda^+_s$ and $\phi^+_s$ are defined analogously to $\lambda^+_k$ and $\phi^+_k$.
...
%
\begin{align}
    m_k(t_{k-1}, z_{s-1}) &=  \delta_{k-1}e_{k-1,s-1} \text{,} \notag \\
    m_k(t_{k-1}, z_{s})   &=  \delta_{k-1}e_{k-1,s} \text{,} \notag \\
    m_k(t_{k}, z_{s-1})   &=  e_{k,s-1} \text{,} \notag \\
    m_k(t_{k}, z_{s})     &=  e_{k,s} \text{.}
\end{align}
%
...
%
\begin{align}
    \dot{v}_k(t,z_{s-1})    &=  (1 - \lambda^+_k(t))^2 v_{k-1,s-1}
                                    + \lambda^+_k(t)^2 v_{k,s-1}
                                    + \sigma^2_k \phi^+_k(t)
                                    \text{,} \notag \\
    \dot{v}_k(t,z_{s})      &=  (1 - \lambda^+_k(t))^2 v_{k-1,s}
                                    + \lambda^+_k(t)^2 v_{k,s}
                                    + \sigma^2_k \phi^+_k(t)
                                    \text{,} \notag \\
    \dot{v}_k(t_{k-1},y)    &=  (1 - \lambda^+_s(y))^2 v_{k-1,s-1}
                                    + \lambda^+_s(y)^2 v_{k-1,s}
                                    + \eta^2 \phi^+_s(y)
                                    \text{,} \notag \\
    \dot{v}_k(t_{k},y)      &=  (1 - \lambda^+_s(y))^2 v_{k,s-1} 
                                    + \lambda^+_s(y)^2 v_{k,s}
                                    + \eta^2 \phi^+_s(y)
    \text{,}
\end{align}
%
each being a one-dimensional Gaussian bridge in its respective dimension.
...
let this check value be at $z_s$ with errors $e_{k-1,s}$ and $e_{k,s}$
and (for simplicity) equal variances $v_s$ at both times.
...
\begin{equation}
    \dot{v}_k(t,y) = \left((1 - \lambda^+_k(t))^2 + \lambda^+_k(t)^2\right)
    \left[
        \left(\frac{y}{z_s}\right)^2 v_s + \eta^2 \frac{y(z_s - y)}{z_s}
    \right]
    + \sigma^2_k \phi^+_k(t)
    \text{;}
\end{equation}
in implementation, we could use the general forms given earlier.   
%

\section{Combined Error}

Recall that the fouling and calibration-drift variances have two components:
%
\begin{equation}
    \dot{v}_k(t) := \operatorname{Var}\!\left[E_t\right] = v_k(t) + \tilde v_k(t)
    \text{,}
\end{equation}
%
...
%
\begin{equation}
    \hat{\sigma}_k^2 = \frac{1}{K}
                       \sum_{k=1}^{K} \frac{r_k^2}{t_k - t_{k-1}}
    \text{;}
\end{equation}
%
...
\end{equation}
%
where $S_k$ is the number of standards used in the $k$-th check.
In operation,
checks commonly use different numbers of standards in order to save time or cost.
...
This estimator is better conditioned than $\hat{\sigma}^2$,
...
During this initial period, parameter estimates can be stabilized
by pooling information across similar devices in a monitoring network.
We propose using an empirical-Bayes approach with conjugate priors
to obtain closed-form shrinkage estimators,
so named because they ``shrink'' or pull the device-specific
parameter estimates toward the network (population) mean.

For brevity, we omit the index $k \in \{f,d\}$ for fouling and drift in this section.
Let $\hat{\sigma}_j^2$ be the maximum-likelihood diffusion estimate for device $j \in \{1,\ldots,J\}$,
and let $K_j$ be the number of checks for device $j$.
Let the population prior hyperparameters be
%
\begin{equation}
    \Theta := {
        \mu_0,\,
...
\end{equation}
%
and assume a normal-inverse-gamma prior on the device-drift rate and diffusion rate,
%
\begin{equation}
    \sigma_j^2 \sim \operatorname{IG}(\alpha_0,\, \beta_0)
    \text{,} \qquad
    \mu_j \mid \sigma_j^2 \sim \mathcal{N}\left(\mu_0,\, \frac{\sigma_j^2}{\kappa_0}\right)
    \text{.}
\end{equation}
%
with mean $\sigma_0^2 := \mathbb{E}[\sigma^2] = \beta_0/(\alpha_0 - 1)$.
%
Omitting the additive constants,
the marginal log-likelihood for $\Theta$ is
%
\begin{equation}
    \ln \mathcal{L} ( \Theta)
    =
    ...
    \left(
        ...
        + \alpha_0 \ln(\beta_0)
        - \ln \Gamma(\alpha_0)
        - \frac{1}{2} \ln\left(\kappa_0 + K_j\right)
        + \ln \Gamma\left(\alpha_0 + \frac{K_j}{2}\right)
          - \left(\alpha_0 + \frac{K_j}{2}\right)
            \ln \beta_j
    \right)
    \label{eq:hierarchical_likelihood_sigma}
    \text{,}
\end{equation}
%
where
...
\begin{equation}
    \beta_j :=  
        \beta_0 + \frac{K_j \hat{\sigma}_j^2}{2}
        + \frac{\kappa_0 K_j (\hat{\mu}_j - \mu_0)^2}
        {2(\kappa_0 + K_j)}
    \text{.}
\end{equation}
%
After maximizing the marginal likelihood to obtain parameter estimates $\hat{\Theta}$,
the empirical-Bayes shrinkage estimator for the drift rate is the posterior mean
%
\begin{equation}
    \tilde{\mu}_j = w_j \hat{\mu}_j + (1 - w_j) \hat{\mu}_0
    \text{,} \qquad
    w_j =  \frac{K_j}
                {K_j + \hat{\kappa}_0}
    \text{,}
\end{equation}
%
and the shrinkage estimator for the diffusion rate is the posterior mean
%
\begin{equation}
   \tilde{\sigma}_j^2 =
   \frac{\hat{\beta}_j}
        {\hat{\alpha}_0 + \frac{K_j}{2} - 1}
    \text{.}            
\end{equation}
%

For the $y$-dimension diffusion parameter $\eta$,
we assume an inverse-gamma prior on the device-diffusion rate,
%
\begin{equation}
    \eta_j^2 \sim \operatorname{IG}(\alpha_0,\, \beta_0)
    \text{,} \qquad
    \alpha_0 > 1
    \text{,} \quad
    \beta_0 > 0
\end{equation}
%
with mean $\eta_0^2 := \mathbb{E}[\eta^2] = \beta_0/(\alpha_0 - 1)$.
Subscripts are omitted for brevity,
but these are not the time-dimension priors
(i.e., $\alpha_{0,\sigma} \ne \alpha_{0,\eta}$).

Let $N_j$ be the total number of standard intervals used to estimate $\hat{\eta}_j^2$
(i.e., the total count of adjacent standard pairs across all checks for device $j$).
Omitting additive constants, the marginal log-likelihood for $(\alpha_0, \beta_0)$ is
%
\begin{equation}
    \ln \mathcal{L} (\alpha_0, \beta_0)
    =
    \sum_{j=1}^{J}
    \left(
        \alpha_0 \ln(\beta_0)
        - \ln \Gamma(\alpha_0)
        - \left(\alpha_0 + \frac{N_j}{2}\right)
                    \ln\left(\beta_0 + \frac{N_j \hat{\eta}_j^2}{2} \right)
        + \ln \Gamma\left(\alpha_0 + \frac{N_j}{2}\right)
    \right)
    \text{.}
    \label{eq:hierarchical_likelihood_eta}
\end{equation}  
%
After maximizing the marginal likelihood to obtain $(\hat{\alpha}_0, \hat{\beta}_0)$,
the empirical-Bayes shrinkage estimator is
%
\begin{equation}
    \tilde{\eta}_j^2 = w_j \hat{\eta}_j^2 + (1 - w_j) \hat{\eta}_0^2
   \text{,} \qquad
    w_j =  \frac{N_j}
                     {N_j + 2(\hat{\alpha}_0 - 1)}
    \text{,}            
\end{equation}
%
where $\hat{\eta}_0^2 := \hat{\beta}_0/(\hat{\alpha}_0 - 1)$.
In practice, the per-device estimates
$\tilde{\mu}_j$, $\tilde{\sigma}_j^2$ and $\tilde{\eta}_j^2$
can be recomputed after each check,
whereas the population hyperparameters
$(\hat{\mu}_0, \hat{\kappa}_0, \hat{\alpha}_0, \hat{\beta}_0)$
may be updated less frequently.
The main limitation of our empirical-Bayes approach is that the inverse-gamma prior
is known to be suboptimal \citep{Gelman_2006}.
Nevertheless, its closed-form shrinkage estimator is advantageous for large, real-time monitoring networks.
Fully Bayesian approaches, while more accurate, are computationally expensive to rerun after each check.
...
Environmental time series are routinely affected by fouling and calibration drift,
...
Later, these check measurements are used to correct the time series
...
to derive confidence intervals for the corrected time series.
