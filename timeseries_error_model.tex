...
\begin{equation}
   \tilde{\sigma}_j^2 = 
   \frac{\hat{\beta}_{\sigma} + \frac{K_j \hat{\sigma}_j^2}{2} 
         + \frac{\hat{\kappa}_{\sigma} K_j (\hat{\mu}_j - \hat{\mu}_0)^2}{2(\hat{\kappa}_{\sigma} + K_j)}}
...
\end{equation}
%
with mean $\mathbb{E}[\eta^2] = \beta_{\eta}/(\alpha_{\eta} - 1)$.

The marginal log-likelihood for $(\alpha_{\eta}, \beta_{\eta})$,
which, omitting additive constants, is
%
\begin{equation}
    \ln \mathcal{L} (\eta \mid \alpha_{\eta}, \beta_{\eta})
    =
    \sum_{j=1}^{J}
    \left(
...
%
\begin{equation}
   \tilde{\eta}_j^2 = w_j \hat{\eta}_j^2 + (1 - w_j) \frac{\hat{\beta}_{\eta}}{\hat{\alpha}_{\eta} - 1}
   \text{,} \qquad
    w_j =  \frac{K_j}
