...
\begin{abstract}
    ...
    Consequently, operators must periodically check, clean, and recalibrate measurement devices.
    By modeling error growth between these checks as a stochastic process,
    we derive bias corrections and confidence intervals for measured time series.
\end{abstract}
...
and propagate their uncertainty through time.
...
that causes systematic error or bias.
Under steady-state conditions,
the fouling error can be assessed by comparing measurements
...
from a colocated reference device.
...
Lastly, any residual error after cleaning and recalibrating the device
is attributed to measurement noise,
which is modeled with a Gaussian distribution,
%
\begin{equation}
...
\end{equation}
%
where $\sigma_p$ represents the precision of the measurement device,
and $\sigma_p^2$ is the variance of the noise,
%
\begin{equation}
    v_p :=
...
Each of the three error components can be modeled on either scale---absolute or fractional---
