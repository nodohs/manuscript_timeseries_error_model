...
\begin{abstract}
    ...
    expected error and its uncertainty, which are used to bias-correct the
...
In practice, these measurements are affected by time-varying errors
from fouling and calibration drift,
as well as irreducible noise due to the precision of the measurement device.
Once deployed, fouling and drift accumulate, and bias the raw measurements.
To correct that bias, the device is checked periodically against known references.
...
We also show how to extend this framework to multi-point calibration procedures and 
...
while remaining consistent with standard field practices for environmental monitoring.
