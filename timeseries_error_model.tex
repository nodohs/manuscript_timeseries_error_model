...
($m^o$, $m^c$, etc.), as
%
TODO This equation needs to include the drift term.
%
\begin{align}
...
\end{align}
%
% m_k(t) = \delta_{k-1} (1 - \lambda^+_k(t)) e_{k-1} + \lambda^+_k(t) e_k + \mu(t - t_{k-1})
...
\end{align}
%
shows that the endpoint uncertainy affects the variance but not the expectation.
%
...
\begin{align}
    ...
    \text{(\ref{eq:variance_uncertain_bridge} adapted for the $y$ dimension),}
\end{align}
%
...
Let $\mathcal{C}_{k,s}$ denote this set of four check errors,
%
\begin{equation}
    \mathcal{C}_{k,s} := \{E_{k-1,s-1} = e_{k-1,s-1},\, E_{k-1,s} = e_{k-1,s},\, 
...
Let $E_f$, $E_d$, and $E_m$ denote independent fractional-error components with
means $m_f,m_d,0$ and variances $v_f,v_d,v_m$, respectively.
The combined fractional error $E$ is given by
%
\begin{equation}
    1+E = (1+E_f)(1+E_d)(1+E_m)
    \text{.}
\end{equation}
%
...
\subsection{Proof}
%
Let $A=1+E_f$, $B=1+E_d$, and $C=1+E_m$, so that $1+E=ABC$.
By independence,
%
\begin{equation}
...
$\mathbb{E}[B^2]=v_d+(1+m_d)^2$, and $\mathbb{E}[C^2]=v_m+1$ (since
