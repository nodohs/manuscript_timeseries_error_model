...
In practice, these measurements have systematic errors
caused by fouling and calibration drift,
...
between the check measurements \citep{Wagner_2006}.
...
%
\begin{enumerate}
    \item The time and value of each check measurement, including any associated reference measurements and standard values.
...
The maximum-likelihood estimators for $\mu$ and $\sigma^2$ can be sensitive to outliers
in the check-measurement errors,
particularly when check intervals are short and residuals are large.
In such cases, a robust estimator based on the median absolute deviation (MAD) may be preferred.
