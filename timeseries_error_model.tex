...
We assume the fouling and calibration-drift erroors evolve as a continuous-time
...
\end{equation}
%
however, this is ill-conditioned if measurement uncertainty is significant,
...
In practice, maximum-likelihood estimators are sensitive to model misspecification,
so if they give poor results, consider some robust alternative
(Appendix~\ref{sec:robust_estimation}).


\section{Monitoring Networks}

After deploying a measurement device, several check measurements may be needed
before drift and diffusion parameters can be estimated reliably.
During this initial period, parameter estimates can be stabilized by
pooling information across similar devices in a monitoring network.
We use an empirical-Bayes approach with conjugate priors that yields closed-form weights
for shrinking device-specific parameter estimates toward population values.
As additional check measurements are collected, population (hyper-)parameters can be updated periodically,
and device-specific parameters and weights can be recomputed after each new check measurement.
Although conjugate priors can be inaccurate \citep{Gelman_2006},
the computational efficiency of this approach simplifies real-time prediction in monitoring
networks comprising thousands of measurement devices.

Devices in the monitoring network are indexed by $j \in \{1,\ldots,J\}$.
The index $k$ (and $K$) is reserved for check-time indexing, and $N_j$ denotes the number
of residual observations available for device $j$,
since a multipoint check yields multiple residuals.

Let $r_{j,1},\ldots,r_{j,N_j}$ denote (approximately) independent, mean-zero residuals for device $j$,
and let
%
\begin{equation}
    s_j^2 := \frac{1}{N_j}\sum_{n=1}^{N_j} r_{j,n}^2
    \text{,}
\end{equation}
%
denote the per-site mean square.
Assume
%
\begin{equation}
    r_{j,n} \mid \sigma_j^2 \sim \mathcal{N}(0,\,\sigma_j^2)
    \text{,}
    \qquad
    \sigma_j^2 \sim \operatorname{IG}(a,b)
    \text{,}
\end{equation}
%
where $\operatorname{IG}(a,b)$ is an inverse-gamma prior on the variance,
with density
%
\begin{equation}
    p(\sigma^2 \mid a,b)
...
\end{equation}
%
and prior mean $\mathbb{E}[\sigma^2]=b/(a-1)$ for $a>1$.

Integrating out $\sigma_j^2$ yields a closed-form marginal likelihood for $(a,b)$.
Up to constants that do not depend on $(a,b)$, the log-marginal likelihood is
%
\begin{equation}
    \ln L(r \mid a,b)
    =
    ...
    \left(
        a\ln(b)
        + \ln \Gamma\left(a + \frac{N_j}{2}\right)
        - \ln \Gamma(a)
        - \left(a + \frac{N_j}{2}\right)\ln\left(b + \frac{N_j s_j^2}{2}\right)
    \right)
    \text{.}
    \label{eq:hierarchical_likelihood}
\end{equation}
%
Empirical-Bayes estimates $\hat{a}$ and $\hat{b}$ can be obtained by maximizing
Equation~\ref{eq:hierarchical_likelihood}.

Given hyperparameters $(a,b)$, the posterior for $\sigma_j^2$ is
$\operatorname{IG}\bigl(a + N_j/2,\; b + N_j s_j^2/2\bigr)$,
so the posterior mean (for $a+N_j/2>1$) is
%
\begin{equation}
    \mathbb{E}[\sigma_j^2 \mid r_{j,1:N_j},a,b]
    =
    \frac{b + \tfrac{N_j}{2}s_j^2}{a + \tfrac{N_j}{2} - 1}
    =
    w_j\,s_j^2 + (1-w_j)\,\bar{\sigma}^2
    \text{,}
\end{equation}
%
where $\bar{\sigma}^2 := b/(a-1)$ and
%
\begin{equation}
    w_j := \frac{N_j}{N_j + 2(a-1)}
    \text{.}
\end{equation}
%
Substituting $(\hat{a},\hat{b})$ gives the empirical-Bayes shrinkage estimate
%
\begin{equation}
    \hat{\sigma}_j^2
    =
    ...
    \text{,}
    \qquad
    \hat{w}_j := \frac{N_j}{N_j + 2(\hat{a}-1)}
    \text{,}
\end{equation}
%
The estimator can be recomputed after each new check measurement.
...
$\mathbb{E}[C]=1$) yeilds Equation~\ref{eq:combined_fractional_variance_proof}.
