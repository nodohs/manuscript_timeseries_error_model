...
This estimator is better conditioned than $\hat{\sigma}$,
...
%
\begin{equation}
    \sigma_j^2 \sim \operatorname{IG}(\alpha_{\sigma},\, \beta_{\sigma})
    \text{,} \qquad
    \mu_j \mid \sigma_j^2 \sim \mathcal{N}\left(\mu_0,\, \frac{\sigma_j^2}{\kappa_{\sigma}}\right)
    \text{.}
\end{equation}
%
Omitting the additive constants,
...
\begin{equation}
    ...
    \sum_{j=1}^{J}
    \left(
        \frac{1}{2} \ln(\kappa_{\sigma})
        + \alpha_{\sigma} \ln(\beta_{\sigma})
        - \ln \Gamma(\alpha_{\sigma})
        - \frac{1}{2} \ln\left(\kappa_{\sigma} + K_j\right)
        + \ln \Gamma\left(\alpha_{\sigma} + \frac{K_j}{2}\right)
          - \left(\alpha_{\sigma} + \frac{K_j}{2}\right)
            \ln \psi_j(\Theta)
    \right)
...
\begin{equation}
    \psi_j(\Theta) :=
                \beta_{\sigma} + \frac{K_j \hat{\sigma}_j^2}{2}
                + \frac{\kappa_{\sigma} K_j (\hat{\mu}_j - \mu_0)^2}
                     {2(\kappa_{\sigma} + K_j)}
    \text{.}
\end{equation}
%
...
\begin{equation}
   \tilde{\sigma}_j^2 = 
   \frac{\hat{\psi}_j(\hat{\Theta})}
        {\hat{\alpha}_{\sigma} + \frac{K_j}{2} - 1}
    \text{.}            
\end{equation}
%
...
%
\begin{equation}
    \eta_j^2 \sim \operatorname{IG}(\alpha_{\eta},\, \beta_{\eta})
    \text{,} \qquad
    \alpha_{\eta} > 1
    \text{,} \quad
    \beta_{\eta} > 0
\end{equation}
%
with mean $\bar{\eta}^2 := \mathbb{E}[\eta^2] = \beta_{\eta}/(\alpha_{\eta} - 1)$.
Omitting additive constants, the marginal log-likelihood for $(\alpha_{\eta}, \beta_{\eta})$ is
%
\begin{equation}
    \ln \mathcal{L} (\alpha_{\eta}, \beta_{\eta})
    =
    \sum_{j=1}^{J}
    \left(
        \alpha_{\eta} \ln(\beta_{\eta})
        - \ln \Gamma(\alpha_{\eta})
        - \left(\alpha_{\eta} + \frac{K_j}{2}\right)
                    \ln\left(\beta_{\eta} + \frac{K_j \hat{\eta}_j^2}{2} \right)
        + \ln \Gamma\left(\alpha_{\eta} + \frac{K_j}{2}\right)
    \right)
    \text{.}
    \label{eq:hierarchical_likelihood_eta}
\end{equation}  
%
After maximizing the marginal likelihood to obtain $(\hat{\alpha}_{\eta}, \hat{\beta}_{\eta})$,
...
\begin{equation}
   ...
   \text{,} \qquad
    w_j =  \frac{K_j}
                {K_j + 2(\hat{\alpha}_{\eta} - 1)}
    \text{,}            
\end{equation}
%
where $\hat{\bar{\eta}}^2 := \hat{\beta}_{\eta}/(\hat{\alpha}_{\eta} - 1)$.

In practice, the per-device estimates
$\hat{\sigma}_j^2$, $\hat{\eta}_j^2$, $\hat{\mu}_j$, $\tilde{\mu}_j$, and $\tilde{\eta}_j^2$
can be recomputed after each check,
whereas the population hyperparameters
$(\hat{\mu}_0, \hat{\kappa}_{\sigma}, \hat{\alpha}_{\sigma}, \hat{\beta}_{\sigma}, \hat{\alpha}_{\eta}, \hat{\beta}_{\eta})$
