...
\begin{equation}
    ...
    \mathcal{N}\left(
        ...
        v^o_k(t_k) + v_k + v_r
    \right)
    \label{eq:error_transition}
    \text{,}
\end{equation}
where $v_k$ is the observational variance,
and $v_r$ is the observational variance at the last reset event (cleaning or recalibration).


By defining the residual and variance terms as
\begin{align}
    r_k &:= e_k - m^o_k(t) \\
    u_k &:= v^o_k(t) + v_k + v_r
    \text{,}
...
\end{equation}
%
Special case with no observational uncertainty,
then the parameters have closed-form estimators
\begin{equation}
