...
We propose using an empirical-Bayes approach with conjugate priors to obtain a closed-form 
expression for the prior, which shrinks the individual estimate toward the network (population) mean.
...
\end{equation}
%
Assume a normal-inverse-gamma prior on the device-drift rate and diffusion rate,
%
\begin{equation}
...
\end{equation}
%
After computing $\hat{\Theta}$,
the empirical-Bayes shrinkage estimator for
the drift rate is
%
\begin{equation}
...
\end{equation}
%
where $\hat{\psi}_j$ is $\psi_j$ evaluated at the estimated hyperparameters.
...
\end{equation}  
%
Given the maximum likelihood estimate for $\hat{\eta}_j^2$,
...
\end{equation}
%
where $\hat{\bar{\eta}}^2$ is the estimated prior mean.
