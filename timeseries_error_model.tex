...
We assume the fouling and calibration-drift errors evolve as a continuous-time
...
The timeseries $E(t)$ is subdivided into intervals that are bracketed by check measurements.
...
we derive the formula for fouling and calibration-drift checks.
...
\end{equation}
%
and using linearity of the expectation,
%
\begin{align}
...
\end{align}
%
shows the expectation is unaffected by the endpoint uncertainty.
%
Because $W_t$ is independent of the check errors $E_{k-1}$ and $E_k$,
the combined variance is given by the variance of a linear combination,
%
\begin{align}
...
\end{equation}
%
which is constant for open intervals,
$ \tilde{v}^o_k(t) = v_{k}(t_k) = v_{k}(t_{k-1})$.
...
%
\begin{equation}
    \operatorname{Corr}\left[E_{z_a},\, E_{z_b}\right] = 0
    \quad \Rightarrow \quad
...
Except now, the boundary condition is not a point but a second bridge between the standards in the $y$ dimension.
...
\begin{align}
    ...
    \text{\ref{eq:variance_uncertain_bridge} as a function of $y$,}
\end{align}
%
...
\end{equation}
%
where $v_k(t)$ is the error-process evolution 
...
\begin{align}
    ...
    \qquad \text{and} \\
    v(t) &=
        \left(\dot{v}_f + (1+m_f)^2\right)
        \left(\dot{v}_d + (1+m_d)^2\right)
        \left(\dot{v}_m + 1\right)
        - \left((1+m_f)(1+m_d)\right)^2
    \qquad \text{(Appendix~\ref{sec:combined_fractional_proof}),}
\end{align}
%
with inputs $(t,y)$ omitted for clarity.
...
Recall that $m(t)$ and $v(t)$ give the mean and variance of the error $E$ at time $t$.
On the absolute scale, the measurement model is $Y = X + E$.
Therefore, the bias-corrected estimate of the measurand $X$ is given by
%
\begin{equation}
    x(t) = y(t) - m(t),
\end{equation}
%
where $y(t)$ is the uncorrected measurement at time $t$.
The errors are conditionally Gaussian,
...
a bias-corrected estimate of $X$ is given by
%
\begin{equation}
...
If measurement errors exceed 100\%,
this equation has a negative lower bound.
If the measurand is strictly positive,
consider using a log-multiplicative error model instead
(Appendix~\ref{sec:log_multiplicative_error}).
...
Maximum-likelihood estimators are sensitive to model misspecification,
so in practice it may be necessary to use a robust alternative 
(Appendix~\ref{sec:robust_estimation}).
...
and any constants $a_1, a_2, \ldots, a_n$, where
%
\begin{equation}
...
\begin{equation}
    ...
    =
    \sum_{i=1}^{n} \sum_{j=1}^{n} a_i a_j \operatorname{Cov}\left[ X_i, X_j \right]
\end{equation}
%
...
\begin{equation}
    \operatorname{Var}\left[ Y \right]
        ...
        + b^2 \operatorname{Var}\left[ Z \right]
        + 2ab \operatorname{Cov}\left[ X, Z \right]
\end{equation}
...
means $m_f,m_d,0$ and variances $v_f,v_d,v_p$, respectively.
The combined fractional error $E$ is given by
%
\begin{equation}
...
%
\begin{equation}
    v := \operatorname{Var}\left[ E \right]
        =
        \left(v_f+(1+m_f)^2\right)
        \left(v_d+(1+m_d)^2\right)
        \left(v_p+1\right)
        -\left((1+m_f)(1+m_d)\right)^2
        \label{eq:combined_fractional_variance_proof}
        \text{.}
\end{equation}
%
...
$\mathbb{E}[C]=1$) yields Equation~\ref{eq:combined_fractional_variance_proof}.
...
%
\begin{equation}
    \varepsilon = \ln Y - \ln X
    \text{.}
\end{equation}
%
Similarly, the log-scale calibration check error is
%
\begin{equation}
    \varepsilon_d = \ln Y_s - \ln Z_s = \ln\left( \frac{Y_s}{Z_s} \right)
\end{equation}
%
and the log-scale fouling check error is
%
\begin{equation}
    \varepsilon_f = \left( \ln Y_b - \ln Y_a \right) - \left( \ln Z_b - \ln Z_a \right)
    = \ln\left( \frac{Y_b}{Y_a} \right) - \ln\left( \frac{Z_b}{Z_a} \right)
    \text{.}
\end{equation}
%

In this case, the log-scale error $\varepsilon_t$ is a continuous-time
stochastic process with additive components ($f$, $d$, and $m$) in log space
(Equations~\ref{eq:total_mean} and \ref{eq:total_variance}).

% TODO MOVE
%Conditioning on the observation $Y=y$ and treating $X$ as fixed,
%the conditional distribution of $\ln X$ is
%%
%\begin{equation}
%    \ln X \mid Y = y
%    \sim
%    \mathcal{N}\left( \ln y - m(t),\, v(t) \right)
%    \text{.}
%\end{equation}
%%

The bias-corrected timeseries is given in terms of the expectation of $\ln X$,
which corresponds to the median of $X$,
%
\begin{equation}
    x_{\operatorname{med}}(t)
    := \exp\left( \mathbb{E}\left[ \ln X \mid Y = y \right] \right)
    = y \exp\bigl( -m(t) \bigr)
    \text{.}
\end{equation}
%
...
\end{equation}
%
where $\tfrac{1}{2}v(t)$ is a transformation correction for the lognormal distribution.

The confidence intervals for the corrected timeseries are multiplicative,
%
\begin{equation}
    \mathrm{CI}
    =
...
particularly for short check intervals
or inaccurate measurement precisions.
If either becomes problematic,
better results may be obtained from robust estimators.
For the drift rate, a robust estimator is the median of the increments,
%
\begin{equation}
...
we use a robust M-estimator that downweights outliers.
Define the adjusted residuals
%
\begin{equation}
    r_k = e_k - \delta_{k-1} e_{k-1} - \tilde{\mu}_k (t_k - t_{k-1})
    \text{,}
\end{equation}
%
...
%
\begin{equation}
    u_k = \sigma^2(t_k - t_{k-1}) + v_{k-1} + v_k
    \text{.}
\end{equation}
%
The robust estimator minimizes the Huber loss function,
%
\begin{equation}
    \tilde{\sigma}_k^2 = \underset{\sigma^2}{\operatorname{argmin}} 
    \sum_{k=1}^{K} \rho\left(\frac{r_k}{\sqrt{u_k}}\right)
    \text{,}
\end{equation}
%
...
\begin{equation}
    \rho(z) = 
    \begin{cases}
        \tfrac{1}{2}z^2, & |z| \leq c \\
        c|z| - \tfrac{1}{2}c^2, & |z| > c
    \end{cases}
    \text{,}
\end{equation}
%
with tuning constant $c \approx 1.345$ chosen to achieve 95\% efficiency
relative to maximum likelihood under the Gaussian model.
This objective is solved iteratively using standard M-estimation algorithms.
The Huber loss behaves quadratically for small residuals (like Gaussian ML)
but linearly for large residuals (robust to outliers),
without requiring a specific parametric form for the tail behavior.
