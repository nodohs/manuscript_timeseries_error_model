...
Note that the reset is ``soft'' in the sense that
...
For notational convenience, we omit the index $k \in {f,d}$ for fouling and drift in this section.
...
Let the population hyperparameters be
%
\begin{equation}
    \Theta := {
        \mu_0,\,
...
\end{equation}
%
After maximizing the marginal likelihood to obtain $\hat{\Theta}$,
...
%
\begin{equation}
    \eta_j^2 \sim \operatorname{IG}(\alpha_1,\, \beta_1)
    \text{,} \qquad
    \alpha_1 > 1
    \text{,} \quad
    \beta_1 > 0
\end{equation}
%
with mean $\bar{\eta}^2 := \mathbb{E}[\eta^2] = \beta_1/(\alpha_1 - 1)$.
...
Omitting additive constants, the marginal log-likelihood for $(\alpha_1, \beta_1)$ is
%
\begin{equation}
    \ln \mathcal{L} (\alpha_1, \beta_1)
    =
    \sum_{j=1}^{J}
    \left(
        \alpha_1 \ln(\beta_1)
        - \ln \Gamma(\alpha_1)
        - \left(\alpha_1 + \frac{N_j}{2}\right)
                    \ln\left(\beta_1 + \frac{N_j \hat{\eta}_j^2}{2} \right)
        + \ln \Gamma\left(\alpha_1 + \frac{N_j}{2}\right)
    \right)
    \text{.}
    \label{eq:hierarchical_likelihood_eta}
\end{equation}  
%
After maximizing the marginal likelihood to obtain $(\hat{\alpha}_1, \hat{\beta}_1)$,
the empirical-Bayes shrinkage estimator is
%
\begin{equation}
   \tilde{\eta}_j^2 = w_j \hat{\eta}_j^2 + (1 - w_j) \hat{\bar{\eta}}^2
   \text{,} \qquad
    w_j =  \frac{N_j}
                {N_j + 2(\hat{\alpha}_1 - 1)}
    \text{,}            
\end{equation}
%
where $\hat{\bar{\eta}}^2 := \hat{\beta}_1/(\hat{\alpha}_1 - 1)$.
...
$(\hat{\mu}_0, \hat{\kappa}_0, \hat{\alpha}_0, \hat{\beta}_0, \hat{\alpha}_1, \hat{\beta}_1)$
