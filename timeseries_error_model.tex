...
$\operatorname{Var}[E_{k-1}] = \operatorname{Var}[E_k]$,
...
$ \tilde{v}^o_k(t) = \operatorname{Var}[E_k]$.

The following sections derive expressions for
$\operatorname{Var}[E_f]$ and $\operatorname{Var}[E_d]$,
which provide the $\operatorname{Var}[E_k]$ term in
Equation~\ref{eq:check_constant_variance}.
Later, when we introduce multipoint calibrations,
the endpoint variances may differ,
so we need the full expression for the check variance
...
(e.g., time averaging),
such that the error are perfectly correlated during the check,
