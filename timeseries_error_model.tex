...
\begin{abstract}
    ...
    By modeling the error growth between checks as a stochastic process,
...
In practice, these measurement devices have systematic errors
caused by fouling and calibration drift,
as well as irreducible noise.
Consequently, the device must be periodically checked against known references,
cleaned, and recalibrated, which resets the systematic error to zero.
Later on, the information from these checks is used to correct
the raw measurements by linearly interpolating errors
between the check measurents \citep{Wagner_2006}.
...
As the checks are also imprecise, we treat them as noisy observations
of the error process and propagate their uncertainty into the time series.
Then we extend the approach to multipoint calibrations
