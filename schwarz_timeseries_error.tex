...
\end{align}
%
where $\lambda^+_s(t)$ is the linear interpolation weight,
extended by
%
\begin{equation}
    \lambda^+_s(t) =
    \begin{cases}
        1, & y < y_{1} \\
        \dfrac{y - y_{s-1}}{y_s - y_{s-1}}, & y \in [y_{s-1}, y_s] \\
        0, & y > y_{S}
    \end{cases}
    \text{,}
\end{equation}
...
Similarly,
the variance is interpolated between the two standards,
%
...
\begin{equation}
    m^c(t,y)
    =
    (1 - \lambda^+_k(t))(1 - \lambda^+_s(y)) e_{k-1,s-1}
    +
    (1 - \lambda^+_k(t)) \lambda^+_s(y) e_{k-1,s}
    +
...
\begin{align}
    ...
    \text{,}
\end{align}
and thereby define the conditional distribution of the error
without using superscripts,
...
v(t) &=
    ...
    \text{,}
\end{align}
where $d$, $f,$ and $p$ denote the drift, fouling, and precision components, respectively.
...
%
\begin{equation}
    x(t) = y_t - m_{\theta}(y_t, t)
\end{equation}
%
where $\theta$ is a set of parameters to be estimated from the check measurements.

Uncertainty in calibration 
for standards
$s \in \{s_1, s_2, \ldots, s_S\}$ 
is
\begin{equation}
    \operatorname{Var} \left[ f (x) \right]
    =
    (1 - x)^2 \sigma_p(x_{s-1})^2 +
    x^2 \sigma_p(x_{s})^2
\end{equation}


Intro Binning.
Confidence interval has the general formula
%
\begin{equation}
    \text{CI} = x(t) \pm q_{1-\alpha/2} \sqrt{v_{b}(y_t, t)}
\end{equation}
%
where $b$ indicates the bin associated with time $t$.
