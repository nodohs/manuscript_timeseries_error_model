...
Any check may determine one or both types of error,
and may or may not involve cleaning or recalibration of the device,
which resets the error to zero.
After each check,
the raw time-series measurements is corrected to account for any fouling and calibration error,
which are propagated through time between the checks points.
The uncertainty of the corrected measurements therefore incorporates any uncertainty in the check measurements,
as well as the uncertainty of the time-dependent correction processes.
...
and that the error is perfectly correlated during the check process.
...
\end{equation}
%
such that the variance induced by the calibration error is
%
\begin{equation}
    \operatorname{Var}\left[ E_p + E_s \right] = \sigma^2_p + \sigma^2_s
    \text{.}
\end{equation}
%
...
Conversely, when $e_{k,s}$ is observed, then
