...
%Recalling that
%\begin{align}
%    m^o_k(t) &:= \delta_{k-1} e_{k-1} + \mu_k (t - t_{k-1}) \\
%    v^o_k(t) &:= \sigma^2_k (t - t_{k-1})
%\end{align}
%and $\tilde{v}_k(t)$ is the additional variance from the check measurements.
By defining the residual and variance terms as
\begin{align}
    r_k &:= e_k - m^o_k(t) \\
    u_k &:= v^o_k(t) + \tilde{v}^o_k(t)
    \text{,}
\end{align}
