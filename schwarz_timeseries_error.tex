...
\begin{abstract}
    ...
    We represent these errors as a continuous-time stochastic processes
    that evolves between maintenance events,
    during which the measurement device is checked, cleaned, and recalibrated.
    Conditioning on these check measurements yields
    closed-form estimates of the expected error and its uncertainty,
    which can be used to compute bias-corrections and confidence intervals
    for the time-series.
\end{abstract}
...
%
Environmental monitoring relies increasingly on in situ measurements 
collected at high temporal resolution over extended deployment periods.
In practice, these measurements are affected by time-varying error
that arise from fouling, calibration drift, as well as the inherent
precision of the measurement device.
The device precision represents the irreducible noise that is present 
even under ideal conditions, when the device is clean and properly calibrated.
Once a clean, and calibrated device is deployed into the environment,
fouling and calibration-drift may accumulate over time and
bias the raw measurements.
To correct this bias, fouling and calibration are checked periodically
by comparing the device against known references.
Each check may quantify one or both error components
and may involve cleaning or recalibration,
which resets the corresponding error to zero.

In this paper, we represent fouling and calibration-drift errors as
continuous-time stochastic processes that evolve gradually between checks.
Conditioning these processes on discrete check measurements yields simple
expressions for the expected error and its uncertainty during a deployment.
The proposed error model is equivalent to a linear Gaussian state-space model,
with Wiener-process evolution between noisy observations,
and the resulting expressions for conditional means and variances
correspond to the closed-form Kalman filtering and smoothing solutions for this system.

The framework extends naturally to multi-point calibration procedures and to
networks comprising multiple measurement devices with shared parameters.
The resulting methods provide transparent, analytically tractable
estimates of corrected measurements and associated confidence intervals, while
remaining consistent with field practices for deploying environmental sensors.
...
($f$) or calibration drift ($d$)—with $k \in \{f,d\}$.

We first consider the period after observing the error at time $t_{k-1}$
but before observing the error at time $t_k$.
We refer to this as an \emph{open interval}, which arises during real-time
uncertainty estimation.
In contrast, a \emph{closed interval} is bracketed by observations at both
...
\end{equation}
%
which simplifies, during open intervals,
...
\end{align}
%
where $\eta^2$ is the diffusion rate in the $y$ dimension.
...
For absolute errors, recall the observation model $Y_t = X_t + E_t$.
Let $m(t)$ and $v(t)$ denote the mean and variance of $E_t$ at time $t$.
Treating $X_t$ as fixed and $E_t$ as independent measurement error,
the bias-corrected estimate of $X_t$ is
\begin{equation}
    x(t) := y_t - m(t),
\end{equation}
...
For fractional errors $\mathcal{E}_t$ with $|\mathcal{E}_t|\ll 1$, 
we use the observation model $Y_t = X_t(1+\mathcal{E}_t)$.
Assuming $\mathcal{E}_t$ is independent of $Y_t$
with mean $m(t)$ and variance $v(t)$,
a bias-corrected estimate is
\begin{equation}
x(t) := y_t\bigl(1-m(t)\bigr)
\text{,}
\end{equation}
%
...
%
\begin{equation}
\text{CI} = x(t) \pm q_{1-\alpha/2}\,y_t\sqrt{v(t)}
\text{.}
\end{equation}
%

When measurement errors exceed 100\% and the measurand is strictly positive,
we may use a log-multiplicitive error model,
described in Appendix~\ref{sec:log_multiplicative_error}.
...
Let $\varepsilon_t$ denote the measurement error
on the logarithmic scale, and assume the observation model
%
\begin{equation}
    Y_t = X_t \exp(\varepsilon_t),
\end{equation}
%
so that
%
\begin{equation}
    \log Y_t = \log X_t + \varepsilon_t.
\end{equation}
%
Here, $\varepsilon_t$ is dimensionless and represents error on the log scale.
As in the additive case, we model $\varepsilon_t$ as a continuous-time stochastic
process and let $m(t)$ and $v(t)$ denote its conditional mean and variance at
time~$t$, as derived above.

Conditioning on the observation $Y_t = y_t$ and treating $X_t$ as fixed,
the conditional distribution of $\log X_t$ is
%
\begin{equation}
    \log X_t \mid Y_t = y_t
    \sim
    \mathcal{N}\!\left( \log y_t - m(t),\; v(t) \right).
\end{equation}
%

A bias-corrected point estimate of $X_t$ may be defined in two common ways.
First, the median of the implied lognormal distribution,
which is unbiased on the logarithmic scale, is given by
%
\begin{equation}
    x_{\mathrm{med}}(t)
    := \exp\!\left( \mathbb{E}\!\left[ \log X_t \mid Y_t = y_t \right] \right)
    = y_t \exp\!\bigl( -m(t) \bigr).
\end{equation}
%
Alternatively, the conditional mean of $X_t$ on the original scale is
%
\begin{equation}
    x_{\mathrm{mean}}(t)
    := \mathbb{E}\!\left[ X_t \mid Y_t = y_t \right]
    = y_t \exp\!\left( -m(t) + \tfrac{1}{2} v(t) \right),
\end{equation}
%
which includes the standard lognormal bias-correction term
$\tfrac{1}{2}v(t)$.

Under the Gaussian assumption for $\varepsilon_t$, an approximate
$(1-\alpha)$ confidence interval for $X_t$ is obtained from the quantiles of
