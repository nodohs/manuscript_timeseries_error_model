...
$k \in \{f, d\}$.
This notation is slighly overloaded but concise.
For example, $\lambda_f(t)$ denotes the fouling weight
...
\begin{align}
    m^o(t) &:=
        ...
        = \delta_{k-1} e_{k-1} + \mu t
        \qquad \text{and}
        \label{eq:wiener_mean}\\
    v^o(t) &:=
        \text{Var}\left[E_t \mid E_{k-1} = e_{k-1} \right]
        = \sigma^2 t
...
\begin{align}
    m^o(t)
        ...
        \qquad \text{and} \\
    v^o(t)
        &:= \text{Var}\left[E_t \mid E_{t_{k-1}} = e_{k-1} \right]
        = \sigma^2_{p}
        \text{.}
...
by the indirect effect of calibration-drift.
In this case, the conditional variance is
%
\begin{align}
    v^c(t)
        &:= \text{Var}\left[E_t \mid E_{t_{k-1}} = e_{k-1} ,\, E_{t_k} = e_k \right]
        = \sigma^2_{p} \lambda_{d}(t) \chi_{d}(t)
        \text{,}
...
%
%The total error variance is the sum of the fouling, drift, and precision components.
In summary,
the affect of drift $d$, fouling $f$, and precision $p$
is modeled by the sum of the relevant means and variances.
Let $m_k(t)$ and $v_k(t)$ be the conditional mean and variance
of the error process at time $t$, respectively,
then combined mean $m(t)$ and variance $v(t)$ are written concisely as
\begin{align}
m(t) &=
    m_{f}(t)
    +
...
v(t) &=
    ...
    \text{,}
\end{align}
where the subscripts $f$, $d$, and $p$ refer to fouling, drift, and precision components, respectively,
and using the appropriate definitions for open
(Equations~\ref{eq:wiener_mean}, \ref{eq:wiener_variance}, \ref{eq:precision_variance_open})
or closed intervals (Equations~\ref{eq:bridge_mean}, \ref{eq:precision_variance_closed},
\ref{eq:bridge_variance}).
%
...
\begin{equation}
    x(t) := \mathbb{E}[X_t \mid Y_t = y_t]
        = y_t \left( 1 - \frac{m_{\%}(t)}{100} \right)
        \text{.}
\end{equation}
%
...
%
\begin{equation}
    \text{CI} = x(t) \pm q_{1-\alpha/2} \, \frac{y_t}{100} \sqrt{v_{\%}(t)}
\text{.}
\end{equation}

\section{Variable Corrections}
