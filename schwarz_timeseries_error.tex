...
\begin{equation}
    ...
    \qquad \text{or} \qquad
    \%e_{p} \sim y \frac{\%\sigma_p}{100} \mathcal{N}\left(0 ,\, 1 \right)
...
for $t \in \left[t_{f-1}, t_f \right]$.
...
In summary,
the affect of drift, fouling and precision ($d$, $f$, and $p$)
...
%
\begin{equation}
    x(t) := \mathbb{E}\!\left[X_t \mid y_t\right]
        \approx y_t - m(t)
        \text{,}
\end{equation}
%
...
In the relative-error case, the measured value is
%
\begin{equation}
    y_t = X_t \left(1 + \frac{E_t}{100}\right),
\end{equation}
%
where $E_t$ is the percent error.
Then, on the original scale,
%
\begin{equation}
    x(t) := \mathbb{E}[X_t \mid y_t]
        = \frac{y_t}{1 + m(t)/100}
        \text{.}
\end{equation}
%
...
%
\begin{equation}
    \text{CI} = x(t) \pm q_{1-\alpha/2} \, \frac{\%\sigma_p}{100} \, y_t
\text{.}
\end{equation}
%
