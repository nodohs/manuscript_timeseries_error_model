...
as a $2\sigma$ (approximately 95\%) confidence interval.
...
$\sigma_p$ and $\sigma_r$, respectively,
and are independent,
\begin{equation}
    E_{p} \sim \mathcal{N}\left(0, \sigma^2_p \right)
    \text{,} \qquad
    E_{r} \sim \mathcal{N}\left(0, \sigma^2_r \right)
    \text{.}
\end{equation}
%

We assume the measuring device uses internal filtering
(e.g., time averaging),
so its error is perfectly correlated before and after the check,
%
\begin{equation}
    Y_b = X_b + E_{y_{b}}
    \text{,} \qquad
...
\end{equation}
%
and the fouling error $E_f$ is given by
%
\begin{equation}
...
\begin{align}
    ...
    \operatorname{Var}[E_f]
        ...
        \notag \\
        &= \operatorname{Var}[E_{z_{b}} - E_{z_{a}}]
\end{align}
%
