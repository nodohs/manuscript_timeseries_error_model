...
Let $z_s$ be the standard values for $s = 1, 2, \ldots, S$,
% end point condition
\begin{equation}
    E_{k-1}(y) := E(y, t_{k-1})
    \text{,}
\end{equation}
%
the conditional mean at time $t_{k-1}$ given the check measurements is
%
\begin{align}
...
The mean and variance during the check become the
boundary conditions for the bridge in the time dimension,
thus the conditional mean is a bilinear interpolation between four points,
