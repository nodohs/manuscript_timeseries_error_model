...
\begin{align}
    ...
    \operatorname{Var} \left[ E_t \mid E_{t_{k-1}} = e_{k-1} ,\, E_{t_k} = e_k \right]
        = \sigma^2 (1 - \lambda_k(t)) \, \lambda_k(t) (t_k - t_{k-1})
        = \sigma^2 \frac{(t - t_{k-1})(t_{k} - t)}{t_{k} - t_{k-1}} 
        \label{eq:bridge_variance}
    \text{.}
\end{align}

% By introducting a bridge factor $\phi_k$,
% \begin{equation}
%     \phi_k(t)
%         = \frac{(t - t_{k-1})(t_{k} - t)}{t_{k} - t_{k-1}}
%         = (1 - \lambda_k(t)) \, \lambda_k(t)
%     \text{,}
% \end{equation}
...
\subsection{Multi-point Calibration}
...
Since lack observations to fully characterize the calibration error as a Wiener process,
...
TODO use $\lambda^+_s(t)$ for the interpolation weight in the mean also,
which matches Figure 11 of Wagner.

We will also estimate the variance by interpolation,
but we need to modify the weight to ensure 
that the variance is non-negative when extrapolating beyond the standard range.
Let $\lambda^+_s(t)$ be the linear interpolation weight,
extended by
%
\begin{equation}
    \lambda^+_s(y) =
    \begin{cases}
        1, & y < y_{1} \\
        \frac{y - y_{s-1}}{y_s - y_{s-1}}, & y \in [y_{s-1}, y_s] \\
        0, & y > y_{S}
    \end{cases}
    \text{,}
\end{equation}
%
such that $\lambda^+_s(t) = 1$ or $0$ when extrapolating
below or above the standard range, respectively.
For reasons that will become clear,
we also define the time-variance factor,
\begin{equation}
    \phi^+_k(t) =
    \begin{cases}
        (t_{1} - t), & t < t_{1} \\
...
Then the variance of the calibration-drift is
%
% TODO change all notation to e_{t_{k-1}} and so forth for clarity
% OR preliminary notes that e_{k-1} = e_{t_{k-1}}
\begin{align}
    v^o(t, y)
    &:=
    ...
    \right]
    \underbrace{\left(t - t_{k-1}\right)}_{\text{time-variance factor}} \notag \\
    &=
    \left[
    \left(1 - \lambda^+_s(y)\right)^2 \sigma^2_{s-1}
    +
...
The closed-interval version of the conditional mean is a bilinear interpolation between four points,
...
\begin{align}
    ...
    %\left[
    %    E_{t,x} \mid
    %    E_{t_{k-1},x_{s-1}} = e_{k-1,s-1} ,,\
    %    E_{t_{k-1},x_s} = e_{k-1,s} ,\,
    %    E_{t_k,x_{s-1}} = e_{k,s-1} ,\,
    %    E_{t_k,x_s} = e_{k,s}
    %\right] \\ 
    &=
    (1-\lambda^k(t),\;\lambda^k(t))
    \begin{bmatrix}
        e_{k-1,s-1} & e_{k-1,s} \\
        e_{k,s-1}   & e_{k,s}
    \end{bmatrix}
...
\begin{align}
    m^c(t,y)
    &=
    (1 - \lambda_k(t))(1 - \lambda_s(y)) e_{k-1,s-1} \notag \\
    &\quad
    + (1 - \lambda_k(t)) \lambda_s(y) e_{k-1,s} \notag \\
    &\quad
...
\begin{align}
    m^c(t_{k-1}, y_{s-1}) &=  e_{k-1,s-1} \text{,} \notag \\
    m^c(t_{k-1}, y_{s})   &=  e_{k-1,s} \text{,} \notag \\
    m^c(t_{k}, y_{s-1})   &=  e_{k,s-1} \text{,} \notag \\
    m^c(t_{k}, y_{s})     &=  e_{k,s} \text{.}
\end{align}
%
The closed-interval conditional variance
%
\begin{align}
    v^c(t, y)
    &:=
...
By substituting $\lambda^+$ for $\lambda$
and $\phi^+$ for the time variance factor,
it is easily verified that the open and closed forms are equivalent,
\begin{align}
    m(t) &= m^c(t) = m^o(t)
    \qquad \text{and} \qquad \\
    v(t) &= v^c(t) = v^o(t)
    \text{,}
\end{align}
and thereby define the error distribution without superscripts,
%Letting $d$, $f,$ and $p$ denote the drift, fouling, and precision components,
\begin{equation}
    p(e_t \mid SET) = \mathcal{N}(m(t), v(t))
\end{equation}
with conditional mean and variance given by the sum of the individual components,
\begin{align}
m(t) &=
    m_{f}(t)
    +
