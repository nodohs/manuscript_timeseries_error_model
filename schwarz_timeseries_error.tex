...
\title{Quantifying time-series uncertainty with Wiener fouling and drift}
% \title{Uncertainty of time-series measurements affected by Wiener fouling and drift}
...
and the error $e$ be the difference between the two,
%
\begin{equation}
    e = y - x
    \text{.}
...
the error may be more accuractely represented in relative terms,
%
\begin{equation}
    e_{\%} = 100 \times \frac{y - x}{x}
    \text{,}
\end{equation}
%
given as a percentage of the true value.
All that follows will apply to either case.

Furthermore, we assume that each measurement is affected by three sources of error:
fouling error $e_f$,
calibration-drift error $e_d$,
and error due to the precision of the measuring device $e_p$,
%
\begin{equation}
    e = e_f + e_d + e_p
    \text{.}
...
Fouling error results from some physical, chemical, or biological buildup on the measurement device
and is determined by the difference between measurements taken before and after cleaning.
...
the fouling error must account for any changes during a cleaning:
%
\begin{equation}
    e_f = (y_a - y_b) - (z_a - z_b)
    \qquad \text{or} \qquad
    e_{\%f} = 100 \times \frac{(y_a - y_b) - (z_a - z_b)}{(y_b)}
...
Calibration drift refers to electronic (or mechanical) drift
in the measuring device since the last time it was calibrated.
The calibration-drift error $e_d$ is determined by comparing the measurement against a known standard $z_s$,
, after correcting for fouling,
%
\begin{equation}
    e_d = y_s - z_s
    \qquad \text{or} \qquad
...
\end{equation}
%
where $z_s$ is the known standard value,
and $y_s$ is the measurement of $z_s$ \citep{Wagner_2006}.

Lastly, the precision error $e_p$ results from the inherent inaccuracy of the measurement device not affected by fouling or drift.
In practice, the precision is either determined in a laboratory
and assumed to be independent and identically distributed for each measurement
%
\begin{equation}
...
\end{equation}
%
In many cases, the measurement precision is determined by the device manufacturer
and reported in its specifications.
For example,
a stated accuracy of $\pm 0.1$ units or $\pm 2$\% of reading,
would correspond to the $2\sigma$ or 95\% confidence interval.
Although not strictly necessary,
we will generally assume the fouling and drift errors have the same
form--relative or absolute--as the precision.

%TODO transition here

\subsection{Wiener Process}

When correcting time series measurements,
a standard practice is to assume the error grows at a constant rate \citep{Wagner_2006}.
We will extend that assumption to modeling the measurement uncertainty.
Specifically, that the drift and fouling errors
follow a Wiener process with an optional drift term,
which represents the average growth rate of the error.
(as opposed to the calibration drift).
In practice, this drift term is necessary
when the 
is relevant 
for devices whose 
devices with some sort of reactive or 
Note, that the drift term $\mu$ represents the 



Let $E_t$ be a random variable representing the error at time $t$,
and $e_t$ be the realization of $E_t$.

Suppose we observe the error at times $0 = t_0 < t_1 < t_2 < \cdots < t_K = T$,
with observed errors $E_{t_k} = e_k$ for $k = 0, 1, \ldots, K$.

Thus $t_k - t_{k-1}$ represents the time elapsed between subsequent observations.
% note substitution of subscript for 

Following some observation 
We will assume fouling and calibration drift grow according to a Wiener process with drift,
given by
%
\begin{equation}
    p(e_t \mid E_{k-1} = e_{k-1})
    =
    \mathcal{N}(e_0 + \mu t ,\; \sigma^2 t)
\end{equation}
%
...
%
where
$e_0$ is the initial error at time 0,
...
%
\begin{align}
    \mathbb{E}\left[E_t \mid E_0 = e_0 \right] = e_0 + \mu t
    \text{,} \\
    \text{Var}\left(E_t \mid E_0 = e_0 \right) = \sigma^2 t
\end{align}
%


A Weiner process that is conditioned to start and end at specified values
...
Let $\lambda(t)$ define the linear interpolation weight:
%
\begin{equation}
...
\end{equation}
%
where the subscript $k$ is a place holder for either error component ($f$ or $d$).
For example, $\lambda_f(t)$ denotes the fouling weight for $t \in \left[t_{f-1}, t_f \right]$.

The conditional distribution of the error process at time $t$,
where $t \in \left[t_{k-1}, t_k \right]$, is
%
\begin{equation}
    p(e_t \mid E_{t_{k-1}} = e_{k-1},\; E_{t_k} = e_k)
    =
    \mathcal{N} \left(
        \left( 1 - \lambda(t) \right)e_{k-1}
        + 
        \lambda(t) e_k,\;
...
\end{align}
where $\chi_{d}$ is a function that is 1 if $t \neq t_{d}$ and 0 otherwise,
\begin{equation}
\chi_{d}(t) =
\begin{cases}
    1, & t \neq t_{d} \\
