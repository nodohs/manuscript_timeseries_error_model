...
\end{equation}
%
First, we consider the conditional distrubtion at time $t_{k-1}$,
which forms a bridge in the $y$ dimension.
In the multi-point calibration,
we will use that first bridge as the boundary condition
for constructing a second bridge in the time dimension.

First, $y$-bridge at time $t_{k-1}$.
Given check-measurement errors $e_{k-1,s-1}$ and $e_{k-1,s}$,
the conditional mean of the $y$-bridge is given by
%
\begin{align}
m_{k-1}(y) &:= \mathbb{E} \left[
                            E_{k-1,y}
                            \mid
                           ...
                           \right]
            \notag \\
            &= (1 - \lambda^+_s(y)) \, \mathbb{E} [E_{k-1,s-1}]
             + \lambda^+_s(y) \, \mathbb{E} [E_{k-1,s}] \notag \\
            &= (1 - \lambda^+_s(y)) e_{k-1,s-1}
             + \lambda^+_s(y) e_{k-1,s}
\label{eq:y_bridge_mean}
\text{,}
\end{align}
%
and the conditional variance of the $y$-bridge is
%
\begin{align}
    v_{k-1}(y) &:= \operatorname{Var} \left[
                            E_{k-1,y}
                            \mid
                            E_{k-1,s-1} = e_{k-1,s-1},\,
                            E_{k-1,s} = e_{k-1,s}
                            \right] \notag \\
               &= (1 - \lambda^+_s(y))^2 \operatorname{Var}[E_{k-1,s-1}]
                + \lambda^+_s(y)^2 \operatorname{Var}[E_{k-1,s}]
                + \eta^2 \phi^+_{s}(m_{k-1}(y))
...
The conditional mean is a bilinear interpolation between four courner points,
...
%
% TODO
The conditional variance is given by plugging the variance of the y-bridge
\ref{eq:y_bridge_variance}
...
For the means,
these are given by Equations~
\ref{eq:unified_wiener_mean},
\ref{eq:multi_point_mean}
...
.
% TODO THERE ARE 5 EQUATIONS TO CITE HERE
