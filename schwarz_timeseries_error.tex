...
%
\begin{equation}
    \varepsilon = \frac{y - x}{y} = \frac{\%e}{100}
...
%
\begin{equation}
    1 + \varepsilon = (1 + \varepsilon_f)(1 + \varepsilon_d)(1 + \varepsilon_p)
    \text{,}
\end{equation}
%
but when the errors are small (less than a few percent),
their relation is approximately additive,
%
\begin{equation}
    \varepsilon \approx \varepsilon_f + \varepsilon_d + \varepsilon_p
    \text{.}
\end{equation}
%

Fouling arises from physical, chemical, or biological buildup that biases the measurement.
It is assessed using check measurements taken immediately
...
\begin{equation}
    e_f = (y_b - y_a) - (z_b - z_a)
    \qquad \text{or} \qquad
    \varepsilon_f = \frac{(y_b - y_a) - (z_b - z_a)}{y_b}
    \label{eq:fouling_error}
    \text{,}
\end{equation}
%
where $z_b$ and $z_a$ are the before-and-after reference measurements
\citep[][noting their convention flips the sign of the error]{Wagner_2006}.
...
\begin{equation}
    e_d = y_s - z_s
    \qquad \text{or} \qquad
    \varepsilon_{d} = \frac{y_s - z_s}{y_s}
...
and the error has a Gaussian distribution,
%
\begin{equation}
    e_p \sim \mathcal{N}(0 ,\, \sigma_p^2)
    \qquad \text{or} \qquad
    \varepsilon_p \sim \mathcal{N}(0 ,\, \sigma_p^2)
    \text{,} \qquad \text{by} \quad
    \sigma_p := \frac{\sigma_{\%p}}{100} 
    \label{eq:precision_error}
    \text{.}
\end{equation}
%
The precision is typically reported in the device specifications,
(sometimes as the "accuracy") and given as a $2\sigma$ (approximately 95\%)
confidence interval.
We assume fouling and drift errors have the same scale (absolute or relative)
...
for fouling and calibration-drift,
...
We assume that drift and fouling errors follow a Wiener process
with an optional drift term $\mu$.
Formally, a Wiener process is a continuous-time stochastic process
whose increments are independent and normally distributed,
with variance proportional to elapsed time.
The drift term $\mu$ represents the mean rate of change over time
(with $\mu=0$ for a classical Wiener process).
This term is important for devices that exhibit systematic bias over time,
such as bias caused by gradual degradation of a consumable element.
...
%
\begin{equation}
    E_t = (1 - \lambda_k(t)) E_{k-1} + \lambda_k(t) E_k + W_t
    \text{,} \qquad
...
%
\begin{align}
    \mathbb{E}[E_t] &= (1 - \lambda_k(t)) \mathbb{E}[E_{k-1}]
                        + \lambda_k(t) \mathbb{E}[E_k] + \mathbb{E}[W_t] \\
                    &= (1 - \lambda_k(t)) m_{k-1} + \lambda_k(t) m_{k}
\end{align}
%
Because $W_t$ is independent of the endpoint errors $E_{k-1}$ and $E_k$,
these terms can be decomposed in the variance,
then using variance of the linear combination yields
%
\begin{align}
    \operatorname{Var}[E_t]
        &= \operatorname{Var}[
            (1 - \lambda_k(t))^2 E_{k-1}
            +
            \lambda_k(t)^2 E_{k}
            ]
            + \operatorname{Var}[W_t] \\
        &= (1 - \lambda_k(t))^2 v_{k-1}
            + \lambda_k(t)^2 v_{k}
            + \sigma^2 \phi^+_k(t)
    \text{.}
\end{align}
...
For a fouling check, we assume the measuring and reference devices have known precisions
of $\sigma_p$ and $\sigma_r$, respectively,
and are independent of each other,
\begin{equation}
    E_{p} \sim \mathcal{N}\left(0, \sigma^2_p \right)
    \text{,} \qquad
    E_{s} \sim \mathcal{N}\left(0, \sigma^2_r \right)
    \text{,}
\end{equation}
%

We will assume that the measuring device uses internal filtering,
like time averaging of the raw signal,
such that the error is perfectly correlated before and after the check,
%
\begin{equation}
    Y_b = X_b + E_{y_{b}}
    \text{,} \qquad
    Y_a = X_a + E_{y_{a}}
    \text{,} \qquad
    \operatorname{Corr} \left( E_{y_{a}}, E_{y_{b}} \right) = 1
\end{equation}
%
\begin{equation}
    E_f = (Y_b + E_{y_{b}}) - (Y_a + E_{y_{a}}) - (Z_b + E_{z_{b}} - Z_a + E_{z_{a}})
\end{equation}
%
then
\begin{align}
    \mathbb{E}[E_f] &= (y_b - y_a) - (z_b - z_a) \\ % EQUATION XXX
    \operatorname{Var}[E_f]
        &= \operatorname{Var}[E_{y_{b}} - E_{y_{a}}] + \operatorname{Var}[E_{z_{b}} - E_{z_{a}}] \\
        &= 0 + \operatorname{Var}[E_{z_{b}} - E_{z_{a}}]
\end{align}
%
...
\begin{equation}
    ...
    \quad \Rightarrow \quad
    \operatorname{Var}[E_f] = 2 \sigma^2_r
    \text{.}
\end{equation}

For a calibration check, we assume the standard and measurement device have known precisions
of $\sigma_p$ and $\sigma_s$, respectively,
and are independent of each other,
%
\begin{equation}
    E_{p} \sim \mathcal{N}\left(0, \sigma^2_p \right)
    \text{,} \qquad
    E_{s} \sim \mathcal{N}\left(0, \sigma^2_s \right)
    \text{,}
\end{equation}
%
\begin{equation}
    E_d = (Y_s + E_{y_{s}}) - (Z_s + E_{z_{s}})
\end{equation}
then
\begin{align}
    \mathbb{E}[E_d] &= y_s - z_s \\ % EQUATION XXX
    \operatorname{Var}\left[ E_{y_{s}} + E_{z_{s}} \right]
        &= \sigma^2_p + \sigma^2_s
\end{align}
...
%
\begin{equation}
    v_c(t) := (1 - \lambda^+_k(t))^2 (\sigma^2_p + \sigma^2_s)
\end{equation}
%
